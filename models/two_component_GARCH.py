...
class ComponentGARCH:
    ...
    def mgf(self, phis, phiq, m):
        a_term, b_term, c_term = self.mgf_terms(phis, phiq, m+1)
        if (self.h is None) or (self.h.shape[0]<m):
            print("generating a new history")
            _, self.h , self.q = self.simulate(n=m+10)
        self.h = np.abs(self.h)
        self.q = np.abs(self.q)
        return np.exp(a_term[m] + b_term[m]*(self.h[m]-self.q[m]) + c_term[m]*self.q[m])
    ...
    def vix_futures(self,T, t=0):
        ...
        return integral_val * 100 * (1/2/np.sqrt(np.pi))